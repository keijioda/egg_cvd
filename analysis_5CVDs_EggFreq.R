...
# There are 19,487 hyperlipidemia cases (48.9%)
ahs_medic %>%
  group_by(HYPERL_YN) %>% 
  tally() %>% 
  mutate(pct = n / sum(n) * 100)

# Find prevalent cases: n = 3579
ahs_medic %>% 
...
# There are 16,239 CVD cases (40.8%)
ahs_medic2 %>% 
  group_by(CVD_YN) %>% 
  tally() %>% 
  mutate(pct = n / sum(n) * 100)

# Find prevalent cases: 3516 prevalent cases
ahs_medic2 %>% 
  filter(CVD_YN == "Yes") %>% 
...
# Yields n = 36,311 subjects
ahs_medic2 <- ahs_medic2 %>% 
  anti_join(prev_cvd, by = "analysisid")  

nrow(ahs_medic2)

# There are 12,723 incident CVD cases (35%)
ahs_medic2 %>% 
  anti_join(prev_cvd) %>% 
  group_by(CVD_YN) %>% 
  tally() %>% 
...
ahs_medic_inc2 %>% 
  CreateTableOne(tablevars, strata = "egg_freq4", data = ., addOverall = TRUE) %>% 
...
vars <- c("bene_sex_F", "rti_race3", "marital", "educyou2", "bmicat", "exercise", "sleephrs2", "smokecat6", "alccat",
          ...
          "kcal100", "eggs_gram_ea_5", "meat_gram_ea", "fish_gram_ea",  "alldairy2_gram_ea",
          "totalveg_gram_ea", "fruits_gram_ea", "refgrains_gram_ea", "whole_mixed_grains_gram_ea",
          "nutsseeds_gram_ea", "legumes_gram_ea"
)

# ahs_medic_inc2 <- ahs_medic_inc2 %>%
#   mutate(CVD_EVER = ymd(CVD_EVER),
#          CVD_YN = ifelse(is.na(CVD_EVER), 0, 1),
#          CVD_YN = factor(CVD_YN, labels = c("No", "Yes"))) %>%
...
mv_mod1 <- coxph(Surv(agein, ageout, inc_CVD) ~ egg_freq4 + 
...
mv_mod2 <- update(mv_mod1, .~. + meat_gram_ea100 + fish_gram_ea100 + alldairy2_gram_ea100 + totalveg_gram_ea100 +
                                ...
                                # nutsseeds_gram_ea100 + legumes_gram_ea100 + eggs_gram_ea_5 * meat_gram_ea100)
...
include_vars <- c("bene_sex_F", "rti_race3", "marital", "educyou2", "bmicat", "exercise", 
                  "sleephrs2", "smokecat6", "alccat", "kcal100")
diet_vars <- c("fish_gram_ea100", "alldairy2_gram_ea100", "totalveg_gram_ea100", "fruits_gram_ea100", 
               "refgrains_gram_ea100", "whole_mixed_grains_gram_ea100", "nutsseeds_gram_ea100", "legumes_gram_ea100")
...
t2 <- tbl_regression(mv_mod2,
                     label = var_labels,
                     include = c(all_of(include_vars), all_of(diet_vars)),
                     add_estimate_to_reference_rows = TRUE,
                     exponentiate = TRUE,
                     pvalue_fun = label_style_pvalue(digits = 3)) %>% 
  add_global_p(keep = FALSE)
...
t3 <- tbl_regression(mv_mod3,
                     label = var_labels,
                     include = c(all_of(include_vars), all_of(diet_vars), starts_with("como_")),
                     add_estimate_to_reference_rows = TRUE,
                     exponentiate = TRUE,
                     pvalue_fun = label_style_pvalue(digits = 3)) %>% 
  add_global_p(keep = FALSE)
...
tbl_merge(tbls = list(t1, t2, t3),
  ...
  modify_header(label = "**Variable**", 
                ...
                p.value_3 = "**p**") %>% 
  # as_flex_table() %>% 
  as_hux_xlsx(file = "./Results/Egg_Freq4_5CVDs_HR_table_MI1.xlsx")

# Checking interactions ---------------------------------------------------

# Egg x meat interaction 
# Significant p = 0.025
mv_mod3 %>% update(.~. + egg_freq4 * meat_gram_ea100) %>% anova()
mv_mod3 %>% update(.~. - meat_gram_ea100 + meat_gram_ea_4 + eggs_gram_ea_5 * meat_gram_ea_4) %>% anova()

# Egg x fish interaction
# Not significant p = 0.38  
mv_mod3 %>% update(.~. + egg_freq4 * fish_gram_ea100) %>% anova()

# Egg x dairy interaction
# Not significant p =  0.39  
mv_mod3 %>% update(.~. + egg_freq4 * alldairy2_gram_ea100) %>% anova()

# Egg x race interaction
# Not significant p = 0.59 
mv_mod3 %>% update(.~. + egg_freq4 * rti_race3) %>% anova()


# Egg x Meat interaction --------------------------------------------------

# mv_mod4 <- update(mv_mod3, .~. - meat_gram_ea100 + meat_gram_ea_4 + eggs_gram_ea_5 * meat_gram_ea_4)
mv_mod4 <- update(mv_mod3, .~. + egg_freq4 * meat_gram_ea100)
summary(mv_mod4)
anova(mv_mod4)

# Function to estimate HR for egg with interaction
intx_hr_egg <- function(model, egg_main_term, egg_meat_intx_term, meat_gram_value){
  L <- coef(model)                          # Copy estimated coefficient
  L[] <- 0                                  # Initialize with zero
  L[egg_main_term] <- 1                     # Replace with 1 for the location of coeffs of interest
  L[egg_meat_intx_term] <- meat_gram_value  # Replace with meat gram for egg x meat intx
  
  est <- coef(model) %*% L
  se  <- sqrt(t(L) %*% vcov(model) %*% L)
...
names(coef(mv_mod4))
egg_labels <- c("1-3/mo", "1-4/wk", "5+/d")

summary(ahs_medic_inc2$meat_gram_ea100)*100

# HR for egg, meat = 0 gram
list(
  intx_hr_egg(mv_mod4, 1, 45, 0),
  intx_hr_egg(mv_mod4, 2, 46, 0),
  intx_hr_egg(mv_mod4, 3, 47, 0)
) %>% 
  do.call(rbind, .) %>% 
  as_tibble() %>% 
  setNames(c("HR", "Lower", "Upper")) %>% 
  mutate(Meat = "0 g/d", Egg  = egg_labels) %>% 
  select(Meat, Egg, everything())

# HR for egg, meat = 10 g/d
list(
  intx_hr_egg(mv_mod4, 1, 45, 10 / 100),
  intx_hr_egg(mv_mod4, 2, 46, 10 / 100),
  intx_hr_egg(mv_mod4, 3, 47, 10 / 100)
) %>% 
  do.call(rbind, .) %>% 
  as_tibble() %>% 
  setNames(c("HR", "Lower", "Upper")) %>% 
  mutate(Meat = "10 g/d", Egg  = egg_labels) %>% 
  select(Meat, Egg, everything())

# HR for egg, meat = 30 g/d
list(
  intx_hr_egg(mv_mod4, 1, 45, 30 / 100),
  intx_hr_egg(mv_mod4, 2, 46, 30 / 100),
  intx_hr_egg(mv_mod4, 3, 47, 30 / 100)
) %>% 
  do.call(rbind, .) %>% 
  as_tibble() %>% 
  setNames(c("HR", "Lower", "Upper")) %>% 
  mutate(Meat = "30 g/d", Egg  = egg_labels) %>% 
  select(Meat, Egg, everything())

# HR for egg, meat = 100 g/d
list(
  intx_hr_egg(mv_mod4, 1, 45, 100 / 100),
  intx_hr_egg(mv_mod4, 2, 46, 100 / 100),
  intx_hr_egg(mv_mod4, 3, 47, 100 / 100)
) %>% 
  do.call(rbind, .) %>% 
  as_tibble() %>% 
  setNames(c("HR", "Lower", "Upper")) %>% 
  mutate(Meat = "100 g/d", Egg  = egg_labels) %>% 
  select(Meat, Egg, everything())


# Function to estimate HR for egg with interaction
intx_hr_eggmeat <- function(model, meat_main_term, egg_meat_intx_term, meat_gram_value, egg_main_term = NULL){
  L <- coef(model)                          # Copy estimated coefficient
  L[] <- 0                                  # Initialize with zero

  if(!is.null(egg_main_term)) {
    L[egg_main_term] <- 1                    # for egg main effect, if needed
    L[egg_meat_intx_term] <- meat_gram_value # for egg x meat intx, replace with zero
    L[meat_main_term] <- meat_gram_value      # for meat main effect, replace with meat gram
  } else {
    L[meat_main_term] <- meat_gram_value      # for meat main effect, replace with meat gram
  }
...
intx_hr_eggmeat(mv_mod4, 27, 45, 0, 1)
intx_hr_eggmeat(mv_mod4, 27, 46, 0, 2)
intx_hr_eggmeat(mv_mod4, 27, 47, 0, 3)

intx_hr_eggmeat(mv_mod4, 27, NULL, 10 / 100)
intx_hr_eggmeat(mv_mod4, 27, 45, 10 / 100, 1)
intx_hr_eggmeat(mv_mod4, 27, 46, 10 / 100, 2)
intx_hr_eggmeat(mv_mod4, 27, 47, 10 / 100, 3)

intx_hr_eggmeat(mv_mod4, 27, NULL, 30 / 100)
intx_hr_eggmeat(mv_mod4, 27, 45, 30 / 100, 1)
intx_hr_eggmeat(mv_mod4, 27, 46, 30 / 100, 2)
intx_hr_eggmeat(mv_mod4, 27, 47, 30 / 100, 3)

intx_hr_eggmeat(mv_mod4, 27, NULL, 100 / 100)
intx_hr_eggmeat(mv_mod4, 27, 45, 100 / 100, 1)
intx_hr_eggmeat(mv_mod4, 27, 46, 100 / 100, 2)
intx_hr_eggmeat(mv_mod4, 27, 47, 100 / 100, 3)


# P-trend for meat intake
# Model4 call:
# Note that there are 4 egg frequency categories
# and meat intake (g/d) as continuous

summary(mv_mod4)
broom::tidy(mv_mod4) %>% filter(str_detect(term, "meat_gram_ea100"))

# Main effect meat_gram_ea100 is the trend test for ref = "None"
# Trend for meat intake at egg freq = "None"
broom::tidy(mv_mod4) %>% filter(term == "meat_gram_ea100")

# Trend for meat intake at egg freq = "1-3x / month"
names(coef(mv_mod4))[c(27, 45)]
coef(mv_mod4)[c(27, 45)]

# Function to get wald statistics and its p-value
get_wald_p <- function(model, coef_loc, coef_wt){
  L <- coef(model)
  L[] <- 0
  L[coef_loc] <- coef_wt
  est <- coef(model) %*% L
...
get_wald_p(mv_mod4, c(27, 45), c(1, 1))

# Trend for meat intake at egg freq = "1-4x / week"
names(coef(mv_mod4))[c(27, 46)]
get_wald_p(mv_mod4, c(27, 46), c(1, 1))

# Trend for meat intake at egg freq = "5x+ / week"
names(coef(mv_mod4))[c(27, 47)]
get_wald_p(mv_mod4, c(27, 47), c(1, 1))

# P-trend for egg freq
ahs_medic_inc2 %>% 
  mutate(test = as.numeric(egg_freq4)) %>% 
  select(egg_freq4, test)

# Replace egg_freq4 with its numeric values, 1, 2, 3, 4
mv_mod5 <- update(mv_mod4, . ~ . - egg_freq4 - egg_freq4:meat_gram_ea100 + as.numeric(egg_freq4) + as.numeric(egg_freq4):meat_gram_ea100)
summary(mv_mod5)

# Main effect as.numeric(egg_freq4) is the trend test for Meat intake = 0
# Trend for egg freq at meat intake = 0
broom::tidy(mv_mod5) %>% filter(str_detect(term, "egg_freq4"))
coef(mv_mod5) %>% names()

# Or alternatively...
get_wald_p(mv_mod5, c(42, 43), c(1, 0))

# Trend for egg freq at meat intake = 10
get_wald_p(mv_mod5, c(42, 43), c(1, 10 / 100))

# Check
L[] <- rep(0, length(coef(mv_mod5)))
L[c(42, 43)] <- c(1, 10/100)
est <- coef(mv_mod5) %*% L
se  <- sqrt(t(L) %*% vcov(mv_mod5) %*% L)
z <- est / se
wald_p <- as.numeric(2 * (1 - pnorm(abs(z))))
wald_p

# Trend for egg freq at meat intake = 30
get_wald_p(mv_mod5, c(42, 43), c(1, 30 / 100))

# Trend for egg freq at meat intake = 100
get_wald_p(mv_mod5, c(42, 43), c(1, 100 / 100))
...
# egg nonlinear p = 0.0001
# fruits nonlinear p = 0.0121
mv_mod3_rcs <- cph(Surv(agein, ageout, inc_CVD) ~ bene_sex_F + rti_race3 + marital + educyou2 + 
                       bmicat + exercise + sleephrs2 + smokecat6 + alccat + kcal100 +
...
mv_mod3_rcs2 <- cph(Surv(agein, ageout, inc_CVD) ~ bene_sex_F + rti_race3 + marital + educyou2 + 
                       bmicat + exercise + sleephrs2 + smokecat6 + alccat + kcal100 +
                       rcs(eggs_gram_ea, parms = 4) + 
                       meat_gram_ea +
                       # rcs(meat_gram_ea, parms = 4) + 
...
Predict(mv_mod3_rcs3, meat_gram_ea = seq(0, 100, by = 5), fun = exp, ref.zero = TRUE) %>% 
  select(meat_gram_ea, yhat, lower, upper)

# pdf("RCS_egg_MV3_MI1.pdf", width = 6.5, height = 5)
Predict(mv_mod3_rcs3, meat_gram_ea = seq(0, 100, by = 1), fun = exp, ref.zero = TRUE) %>% 
  ggplot() +
  geom_line(linewidth = 1.3) +
  scale_y_continuous(breaks = 5:16 / 10) +
  geom_hline(yintercept =  1, linetype = 2) +
  coord_cartesian(ylim = c(0.9, 1.5)) +
  labs(x = "Meat intake (energy-adjusted, gram/day)",
       y = "Adjusted hazard ratio (95% CI)",
       caption = "",
...
# Change the reference to 0 g/d
dd$limits$fruits_gram_ea[2] <- 0
mv_mod3_rcs3 <- update(mv_mod3_rcs2)

Predict(mv_mod3_rcs3, fruits_gram_ea = seq(0, 400, by = 20), fun = exp, ref.zero = TRUE) %>% 
  select(fruits_gram_ea, yhat, lower, upper)

# pdf("RCS_egg_MV3_MI1.pdf", width = 6.5, height = 5)
Predict(mv_mod3_rcs3, fruits_gram_ea = seq(0, 400, by = 1), fun = exp, ref.zero = TRUE) %>% 
  ggplot() +
  geom_line(linewidth = 1.3) +
  scale_y_continuous(breaks = 5:16 / 10) +
  geom_hline(yintercept =  1, linetype = 2) +
  coord_cartesian(ylim = c(0.7, 1.1)) +
  labs(x = "Fruit intake (energy-adjusted, gram/day)",
       y = "Adjusted hazard ratio (95% CI)",
       caption = "",
       title = "Model 3: Cubic spline for fruit intake (Ref = 0 g/d)") +
  theme(text=element_text(size = 14))
# dev.off()
...
# Interaction not significant p = 0.4320  
