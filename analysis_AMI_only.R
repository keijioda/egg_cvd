...
# Identify afib cases
ahs_medic <- ahs_medic %>% 
  mutate(AMI_YN = ifelse(is.na(AMI_EVER), 0, 1),
         AMI_YN = factor(AMI_YN, label = c("No", "Yes")))   

# There are 1857 acute MI cases (4.7%)
ahs_medic %>%
  group_by(AMI_YN) %>% 
  tally() %>% 
  mutate(pct = n / sum(n) * 100)

# Find prevalent cases: 157 prevalent cases
ahs_medic %>% 
  filter(AMI_YN == "Yes") %>% 
  mutate(AMI_EVER = ymd(AMI_EVER)) %>%
  filter(AMI_EVER <= qreturndate) %>% 
  select(analysisid, AMI_YN, qreturndate, AMI_EVER)
...
ahs_medic2 <- ahs_medic %>% 
  mutate(AMI_EVER = ymd(AMI_EVER), 
         AMI_YN = ifelse(is.na(AMI_EVER), 0, 1),
         AMI_YN = factor(AMI_YN, labels = c("No", "Yes")))
...
ahs_medic_inc <- ahs_medic2 %>% 
  mutate(
    ...
    ageout = case_when(
              AMI_YN == "Yes" ~ time_length(interval(BENE_BIRTH_DT, AMI_EVER), "year"),
              AMI_YN == "No" & !is.na(BENE_DEATH_DT)  ~ time_length(interval(BENE_BIRTH_DT, BENE_DEATH_DT), "year"),
              AMI_YN == "No" &  is.na(BENE_DEATH_DT)  ~ age_last_seen),
...
# Mean/median follow-up years: Mean 16.8 years, Median 18.5 years
summary(ahs_medic_inc$fuyear) %>% round(2)
sum(ahs_medic_inc$fuyear)

# Age at diagnosis: Mean 81.9 years, Median 82.4 years
ahs_medic_inc %>% 
  filter(AMI_YN == "Yes") %>% 
  select(ageout) %>% 
  summary()
...
out <- ahs_medic_inc2 %>%
  mutate(AMI_YN2 = fct_recode(AMI_YN, "Non-case" = "No", "Case" = "Yes")) %>%
  CreateTableOne(tablevars, strata = "AMI_YN2", data = ., addOverall = TRUE)
...
ahs_medic_inc2_td %>% 
  ...
  # filter(HYPERL_YN == "No") %>%
  filter(HYPERL_YN == "Yes") %>%
  select(analysisid, tstart, tstop, inc_AMI, hyperl_age, hyperl, HYPERL_EVER, STROKE_TIA_EVER)

ahs_medic_inc2_td %>% 
  as_tibble() %>% 
  filter(HYPERL_YN == "Yes") %>%
  filter(hyperl_age < agein) %>%
  select(analysisid, agein, tstart, tstop, inc_AMI, hyperl_age, hyperl, HYPERL_EVER, STROKE_TIA_EVER)

# Cox proportinal hazards model
coxm <- function(var, dsn = ahs_medic_inc2_td){
  fm <- formula(Surv(tstart, tstop, inc_AMI) ~ var)
...
mv_mod1 <- coxph(Surv(agein, ageout, inc_AMI) ~ eggs_gram_ea_5 + 
...
mv_mod3_rcs <- cph(Surv(agein, ageout, inc_AMI) ~ bene_sex_F + rti_race3 + marital + educyou2 + 
...
mv_mod3_rcs2 <- cph(Surv(agein, ageout, inc_AMI) ~ bene_sex_F + rti_race3 + marital + educyou2 + 
                       bmicat + exercise + sleephrs2 + smokecat6 + alccat + kcal100 +
                       rcs(eggs_gram_ea, parms = 4) + 
                       meat_gram_ea + 
                       fish_gram_ea + 
...
mv_mod <- coxph(Surv(agein, ageout, inc_AMI) ~ bene_sex_F + rti_race3 + marital + educyou2 + 
